...
##############importing the data#########
path = "C:/Users/ahmad/OneDrive/BrandeisUniversity/__Research_120__/Disease_Data/"
...
www = which(rownames(glom_all) %in% rownames(top[1:10000,]))
...
output_directory <- "characteristic_direction_input"
...
for (disease in disease_groups) {
  ...
  combined_data <- cbind(healthy_data, expression_data[, disease_indices])
  ...
  output_data <- data.frame(Labels = labels, t(combined_data))
  
  # Set the gene names as row names
  rownames(output_data) <- rownames(expression_data)
  
  # Generate the output file name
  output_file <- file.path(output_directory, paste0(gsub("[^[:alnum:]]", "_", disease), "_vs_healthy.txt"))
  
  # Export the data frame as a text file
  write.table(output_data, file = output_file, sep = "\t", quote = FALSE, row.names = TRUE)
